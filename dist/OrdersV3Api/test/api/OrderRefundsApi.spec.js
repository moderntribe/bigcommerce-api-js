...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    factory(root.expect, root.OrdersV3);
  }
})(undefined, function (expect, OrdersV3, BCApiClient, config) {
  'use strict';

  var instance;

  beforeEach(function () {
    instance = new OrdersV3.OrderRefundsApi(new BCApiClient(config));
  });

  describe('(package)', function () {
    describe('OrderRefundsApi', function () {
      describe('getorderrefunds', function () {
        it('should call getorderrefunds successfully', _asyncToGenerator(function* () {
          // const getorderrefundsResponse = await instance.getorderrefunds();
          // expect(getorderrefundsResponse.ok).to.be(true);
          // expect(getorderrefundsResponse.body.data).to.be.an('array');
        }));
      });
      describe('postrefund', function () {
        it('should call postrefund successfully', _asyncToGenerator(function* () {
          // const postrefundResponse = await instance.postrefund();
          // expect(postrefundResponse.ok).to.be(true);
          // expect(postrefundResponse.body.data).to.be.an('array');
        }));
      });
      describe('postrefundquote', function () {
        it('should call postrefundquote successfully', _asyncToGenerator(function* () {
          // const postrefundquoteResponse = await instance.postrefundquote();
          // expect(postrefundquoteResponse.ok).to.be(true);
          // expect(postrefundquoteResponse.body.data).to.be.an('array');
        }));
      });
    });
  });
